...
@property (nonatomic, strong) UIScrollView *scrollView; // 底层滚动视图
...
- (void)viewDidLoad {
    ...
    // 创建上导航和下导航视图
    [self creatNavigationBarView]; // 创建上导航栏视图
    [self creatFinishBarView];  // 创建完成状态栏视图
  
    // 设置滚动视图数据
    [self setScrollViewContentDataSource];  // 设置内容数据
    [self configContentViews]; // UI展示数据
}
...
- (void)creatFinishBarView {
    // 下导航背景视图
    UIView *finishView = [[UIView alloc] initWithFrame:CGRectMake(0, ScreenHeight - 49, ScreenWidth, 49)];
    finishView.tag = 4002;
...
- (void)selectMaxNum {
    if ([UIAlertController class] && [UIAlertAction class]) {
        UIAlertController *alert = [UIAlertController alertControllerWithTitle:nil message:[NSString stringWithFormat:@"您最多只能选择%ld张照片", self.imagePickerVC.maxNumOfSelection] preferredStyle:UIAlertControllerStyleAlert];
        UIAlertAction *cancelAction = [UIAlertAction actionWithTitle:@"我知道了" style:UIAlertActionStyleCancel handler:nil];
        [alert addAction:cancelAction];
        [self presentViewController:alert animated:YES completion:nil];
    } else {
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:nil message:[NSString stringWithFormat:@"您最多只能选择%ld张照片", self.imagePickerVC.maxNumOfSelection] delegate:nil cancelButtonTitle:@"我知道了" otherButtonTitles:nil, nil];
        [alert show];
    }
}
